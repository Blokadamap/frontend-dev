...
class AuthorService {
    async get(): Promise<AuthorShort[]> {
        const response = await axiosPublic.get<AuthorShortFromApi[]>(`/authors/`);
    ...
    async create(data: AuthorCreate): Promise<AuthorResponse> {
        ...
        const response = await axiosPrivate.post<AuthorResponseFromApi>(`/authors/`, author);

        return authorMapper.toAuthorResponse(response.data);
    }

    async getFilters(): Promise<AuthorFilters> {
        const response = await axiosPublic.get<AuthorFiltersFromApi>(`/authors/filters`);

        return authorMapper.toAuthorFilters(response.data);
    }

    async getById(id: string): Promise<AuthorResponse> {
        const response = await axiosPublic.get<AuthorResponseFromApi>(`/authors/${id}`);
